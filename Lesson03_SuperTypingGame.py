...
bgm = pygame.mixer.music.load("./Sounds/bgm01.mp3")
pygame.mixer.music.play()
pygame.mixer.music.set_endevent(pygame.USEREVENT+1)
Key_Wav = pygame.mixer.Sound("./Sounds/Key.ogg")
...
while True:
    for event in pygame.event.get():        #游戏退出
        if event.type == (pygame.USEREVENT+1):
            if music_que == 0:
                bgm = pygame.mixer.music.load("./Sounds/bgm02.mp3")
                music_que += 1
            elif music_que == 1:
                bgm = pygame.mixer.music.load("./Sounds/bgm03.mp3")
                music_que += 1
            else:
                bgm = pygame.mixer.music.load("./Sounds/bgm01.mp3")
                music_que = 0
    ...
    #清屏
    if ((all_cnt%330) < 55):                #随着分数变动背景
        bgp = "./background/01.jpg"
    elif((all_cnt%330) < 110):
        bgp = "./background/02.jpg"
    elif ((all_cnt%330) < 165):
        bgp = "./background/03.jpg"
    elif ((all_cnt%330) < 220):
